...
How to obtain Auth0 credentials and initialize applications for the Canmore Stays monorepo.

## Overview

| Resource | Type | Used By | Purpose |
|----------|------|---------|---------|
| **Canmore Stays Web** | SPA | Next.js web | PKCE login flow, no secret needed |
| **Canmore Stays API** | API Resource Server | NestJS API | JWT validation, audience identifier |
...
2. Create tenant (e.g. `canmorestays-core`)
3. Note **tenant domain**: `canmorestays-core.jp.auth0.com`
...
3. Create SPA app ("Canmore Stays Web") → outputs `CLIENT_ID`
...
**`apps/api/.env`:**
```env
AUTH0_DOMAIN=canmorestays-core.jp.auth0.com
AUTH0_AUDIENCE=https://api.canmorestays.com
...
```env
NEXT_PUBLIC_AUTH0_DOMAIN=canmorestays-core.jp.auth0.com
NEXT_PUBLIC_AUTH0_CLIENT_ID=<spa_client_id>
NEXT_PUBLIC_AUTH0_AUDIENCE=https://api.canmorestays.com
```
